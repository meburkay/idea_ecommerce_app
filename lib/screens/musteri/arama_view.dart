...
class _AramaState extends State<Arama> {
  ...
  void buildSearch(String query) {
    if (query.isNotEmpty) {
      productsSearch = products
          .where((element) =>
              element.isim.toLowerCase().contains(query.toLowerCase()))
          .toList();
    } else {
      productsSearch = products;
    }
            ...
            TextField(
              ...
              onSubmitted: (value) {
                Provider.of<AramaViewModel>(context, listen: false)
                    .gecmisBilgisiKaydetme(value);
               
              },
              controller: tfArama,
              autofocus: true,
              decoration: InputDecoration(
                  prefixIcon: Icon(Icons.search), hintText: searchHintText),
            ),
            SizedBox(
              height: 10,
            ),
      ...
      child: GridView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          return GestureDetector(
            onTap: () {
              RouteHelper.goRoute(
                  context: context, page: urunEkrani(productsSearch[index]));
            },
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
  ...
  Image image(int index) =>
      Image.network("${productsSearch[index].urunResimleriUrl[0]}",
          fit: BoxFit.contain);
}
